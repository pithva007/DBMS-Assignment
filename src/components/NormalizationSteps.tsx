...
import { NormalizationStep, NormalizationResult } from '../types/normalization';
...
const NormalizationSteps: React.FC<NormalizationStepsProps> = ({ steps, result }) => {
  return (
    <div className="mt-8 space-y-6">
      ...
      {steps.map((step, idx) => (
        <NormalizationStepCard key={idx} step={step} result={result} />
      ))}
    </div>
  );
};

const NormalizationStepCard: React.FC<{ step: NormalizationStep; result?: NormalizationResult }> = ({ step, result }) => {
  const [isExpanded, setIsExpanded] = useState(true);

  // Determine explanation override for 2NF satisfied
  let displayExplanation = step.explanation;
  if (step.normalForm === '2NF' && step.status === 'satisfied' && result) {
    if (result.candidateKeys.every(k => k.length === 1)) {
      const kStr = result.candidateKeys.map(k => `{${k.join(',')}}`).join(', ');
      displayExplanation = `2NF is automatically satisfied. The candidate key ${kStr} is a single attribute — no proper subsets exist, so partial dependencies are impossible by definition.`;
    } else {
      displayExplanation = `2NF is satisfied. All non-prime attributes are fully functionally dependent on every candidate key. No partial dependencies detected.`;
    }
  }
