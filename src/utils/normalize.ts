...
export const findTransitiveDependencies = (
  ...
  for (const fd of fds) {
    ...
    if (!isLhsSuperkey && nonPrimeRHS.length > 0) {
      ...
      if (!isPartial) {
        // Find which candidate key K determines X
        const K = candidateKeys.find(key => isSubset(fd.lhs, computeClosure(key, fds)));
        const KStr = K ? `{${K.join(',')}}` : '{K}';

        violations.push({
          type: 'transitive',
          fd,
          explanation: `Transitive chain: ${KStr} → {${fd.lhs.join(',')}} → {${nonPrimeRHS.join(',')}}\n{${fd.lhs.join(',')}} is not a superkey and {${nonPrimeRHS.join(',')}} is non-prime.\nViolates 3NF — extract {${fd.lhs.join(',')}} → {${nonPrimeRHS.join(',')}} into a separate relation.`
        });
      }
    }
  }
  ...
  for (const fd of fds) {
    ...
    if (!isSuperkey(fd.lhs, allAttrs, fds)) {
      violations.push({
        type: 'bcnf',
        fd,
        explanation: `{${fd.lhs.join(',')}} → {${fd.rhs.join(',')}} violates BCNF because {${fd.lhs.join(',')}} is not a superkey of the relation. For BCNF, every determinant must be a superkey. Fix: decompose into ({${fd.lhs.join(',')}} ∪ {${fd.rhs.join(',')}}) and (R − {${fd.rhs.join(',')}} + {${fd.lhs.join(',')}})`
      });
    }
  }
  return violations;
